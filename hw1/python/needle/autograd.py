...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
      #final gradient of the node.
      node.grad = sum_node_list(node_to_output_grads_list[node])
      if node.is_leaf():
        continue
      #compute the partials for its parent.
      #         (adjoint for parent, node.op.output_value)
      partials = node.op.gradient_as_tuple(node.grad,node)
      #partials[i] = node.inputs[i]
      
      for (i, parent) in enumerate(node.inputs):
        if parent not in node_to_output_grads_list:
          node_to_output_grads_list[parent] = []
  
        node_to_output_grads_list[parent].append(partials[i])
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for node in node_list:
        if visited.get(node) is None:
          visited[node] = True
          topo_sort_dfs(node, visited, ordering)
    return ordering
